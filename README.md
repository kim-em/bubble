...
- **GET-only upstream cache**: Public Mathlib and explicitly configured Lake downloads also pass through the host-global cache at `~/.bubble/artifact-cache/`. Supported routes are revision- or content-addressed and therefore immutable; replacing data at an already-published revision URL is unsupported (clear the host cache if an upstream violates that contract). The cache is bounded to 50 GiB by default, validates per-container route grants, follows no redirects, accepts no query-selected origins, and rejects every upload method. Manage it with `bubble cache`; customize it with `[artifact_cache] max_size = "100GiB"`, or disable automatic Mathlib proxy setup with `[artifact_cache] enabled = false`. Mathlib can still download directly when proxy setup is unavailable; an explicit `--lake-cache-service` fails closed because Bubble cannot otherwise configure that named service safely.
...
codex = "auto"
kiro = "auto"       # Kiro CLI (native, checksum-pinned bundle)
vibe = "auto"       # Mistral Vibe / Leanstral (Lean 4 agent)
```

### Tools

Coding agents like Claude Code, OpenAI Codex, Kiro CLI, and Mistral Vibe (the home of the Leanstral Lean 4 agent) can be installed in container images. Each tool defaults to `"auto"`, which installs it if the corresponding command is found on your host.
