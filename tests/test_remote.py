...
class TestRemoteOpenFlagForwarding:
    ...
    def test_ai_prompt_uses_stdin_flag(self):
        """AI prompt triggers --ai-prompt-stdin flag (prompt sent via stdin)."""
        cmd_str = self._run_remote_open(ai_prompt="Fix the bug")
        assert "--ai-prompt-stdin" in cmd_str
        # Prompt text should NOT appear in command line (sent via stdin instead)
        assert "Fix the bug" not in cmd_str

    def test_no_ai_prompt_by_default(self):
        cmd_str = self._run_remote_open()
        assert "--ai-prompt-stdin" not in cmd_str
